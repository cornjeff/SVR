...
function welcomeHtml(name: string, password: string) {
  return `<!DOCTYPE html>
<html><body style="margin:0;padding:0;background:#f5f0e8;font-family:Georgia,serif;">
<div style="max-width:560px;margin:40px auto;background:#0d1b2a;border:1px solid #2a3d54;">
  <div style="padding:32px 40px;border-bottom:1px solid #2a3d54;">
  ...
  <div style="padding:32px 40px;">
    ...
    <p style="color:#b8c4d0;font-size:15px;line-height:1.7;margin:0 0 24px;">Your access to the Spruce Valley Ranch Homeowner Portal has been set up. You can now view community announcements, the directory, HOA documents, financials, and reserve time at the Trap &amp; Skeet Range.</p>
    <div style="background:#162032;border:1px solid #2a3d54;padding:20px 24px;margin-bottom:24px;">
      <div style="color:#c9a84c;font-size:11px;letter-spacing:0.15em;text-transform:uppercase;margin-bottom:14px;">Your Portal Access</div>
      <div style="margin-bottom:14px;">
        <div style="color:#6a7f94;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px;">Portal URL</div>
        <a href="${PORTAL_URL}" style="color:#c9a84c;font-size:14px;text-decoration:none;">${PORTAL_URL}</a>
      </div>
      <div>
        <div style="color:#6a7f94;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px;">Password</div>
        <div style="color:#f5f0e8;font-size:18px;letter-spacing:0.08em;font-family:monospace;">${password}</div>
      </div>
    </div>
    <p style="color:#b8c4d0;font-size:14px;line-height:1.7;margin:0 0 8px;">Click <strong style="color:#f5f0e8;">Homeowner Login</strong> on the portal and enter the password above to get started.</p>
    <p style="margin:24px 0 0;"><a href="${PORTAL_URL}" style="background:#c9a84c;color:#0d1b2a;text-decoration:none;padding:10px 22px;font-size:13px;letter-spacing:0.08em;display:inline-block;">Access the Portal</a></p>
    <p style="color:#6a7f94;font-size:12px;margin:28px 0 0;">Spruce Valley Ranch &middot; Blue River, Colorado</p>
  </div>
</div>
</body></html>`
...
serve(async (req) => {
  ...
  try {
    const { type, to, name, date, hour, password } = await req.json()
    ...
    if (type === 'booking') {
      ...
      payload = {
        ...
        html: welcomeHtml(name, password),
      }
    } else {
